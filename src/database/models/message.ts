...
export class MessageModel {
  ...
  update = async (id: string, { imageList, ...message }: Partial<UpdateMessageParams>) => {
    return this.db.transaction(async (trx) => {
      // 1. insert message files (only for valid file IDs that exist in the files table)
      if (imageList && imageList.length > 0) {
        // Filter out temporary IDs (like tmp_img_*) and base64 data URLs
        // Only insert file relationships for actual files in the database
        const validFileIds = imageList.filter((file) => {
          // Skip temporary IDs generated by OpenAI Responses API
          if (file.id.startsWith('tmp_img_')) return false;
          // Skip base64 data URLs (they don't need file table references)
        ...
        if (validFileIds.length > 0) {
          await trx
            .insert(messagesFiles)
            .values(
              validFileIds.map((file) => ({ fileId: file.id, messageId: id, userId: this.userId })),
            );
        }
      }
