...
-- Each CREATE INDEX is wrapped in a DO block so the statement is silently skipped when pg_search
-- is not installed (e.g. Supabase). DROP INDEX IF EXISTS is safe without pg_search.
